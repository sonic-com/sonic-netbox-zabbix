"""Orchestrator that syncs Netbox device/VM data into Zabbix hosts."""
...
from netbox_zabbix.zabbix import ZabbixClient


class SonicNetboxZabbix:
    """Tools for setting up hosts in Zabbix based on Netbox data."""
    ...
    def __init__(self):
        ...
        self.log.debug("Starting Sonic Netbox Zabbix Sync")

        self._zabbix_login()
        self._netbox_login()

    def _zabbix_login(self):
        """Log into Zabbix, creating self.zabbix."""
        self.log.debug("Logging into Zabbix")
        self.zabbix = ZabbixClient(self.log, self.config)

    def _netbox_login(self):
        """Log into Netbox, creating self.netbox."""
        self.log.debug("Logging into Netbox")
        self.netbox = NetboxClient(self.log, self.config)
        ...
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                ...
                if "update_group" in srv.custom_fields and srv.custom_fields["update_group"]:
                    self.log.info(f"Adding update_group to zabbix macro for {name}")
                    ...
                    self.log.warning(f"No update_group for {name}")

                # Populate some macros with netbox tag info:
                self.log.info(f"{name}: Updating $NETBOX.TAGS")
                tags = []
                if srv.tags:
                    for tag in srv.tags:
    ...
    def copy_netbox_info_to_zabbix_tags(self, netbox_servers, zabbix_servers):
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE: tags for {name}")
                srv = netbox_servers[name]
                if "tags" in zabbix_servers[name]:
                    tags = zabbix_servers[name]["tags"]
                    self.log.debug(f"{name} tags(original): {pformat(tags)}")
                    tags = [item for item in tags if not item["tag"].startswith("netbox-")]
                else:
                    tags = []
                self.log.debug(f"{name} tags(1): {pformat(tags)}")
                ...
                self.log.debug(f"{name} tags(2): {pformat(tags)}")
                ...
                self.log.debug(f"{name} tags(3): {pformat(tags)}")
                ...
                if srv.tags:
                    self.log.info(f"{name}: Updating tags")
                    ...
                    self.log.debug(f"{name} tags(4): {pformat(tags)}")

                else:
                    self.log.warning(f"{name}: No netbox tags")
                    self.log.debug(f"{name} srv.tags {pformat(dict(srv.tags))}")
                ...
                if "update_group" in srv.custom_fields and srv.custom_fields["update_group"]:
                    ...
                    self.log.warning(f"{name}: No update_group for")
                ...
                self.log.warning(f"{name}: No such server in {name} in netbox data")

    def copy_netbox_info_to_zabbix_inventory(self, netbox_servers, zabbix_servers):
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE:{name}: inventory")
                srv = netbox_servers[name]
                inventory = {}
                ...
                if srv.site:
                    ...
                    self.log.debug(f"Site Details: {pformat(dict(srv.site))}")
                    ...
                    self.log.debug(f"site_info: {site_info}")
                ...
                if self.netbox.is_physical(srv):
                    ...
                    if srv.location:
                        ...
                        self.log.debug(f"Location Details: {pformat(dict(srv.location))}")
                        location_info = []
                ...
                self.log.warning(f"{name}: No such host in netbox data")
    ...
    def copy_netbox_info_to_zabbix_hostgroups(self, zabbix_servers, netbox_servers):
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE:{name}:groups")
                nbsrv = netbox_servers[name]
                zbsrv = zabbix_servers[name]
                hostgroups = zbsrv["hostgroups"]
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE:{name}: hostgroups:unfiltered: {hostgroups}")
                ...
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE:{name}: hostgroups:filtered: {hostgroups}")

                if any(grp["name"] == "ALL" for grp in hostgroups):
                    self.log.debug("Hostgroup ALL already present")
                else:
                    self.log.info(f"Adding Hostgroup ALL to {name}")
                    new_hostgroup = self.zabbix.hostgroup_get_or_create("ALL")
                    hostgroups.append(new_hostgroup)

                if self.netbox.is_physical(nbsrv):
                    self.log.info(f"Adding Hostgroup Physical Devices to {name}")
                    new_hostgroup = self.zabbix.hostgroup_get_or_create("Physical Devices")
                    hostgroups.append(new_hostgroup)
                else:
                    self.log.debug(f"Virtual Server: {name}")
                ...
                if site:
                    ...
                    self.log.debug(f"{name}:hostgroup_path: {hostgroup_path}")
                    new_hostgroup = self.zabbix.hostgroup_get_or_create(hostgroup_path)
                    self.log.debug(f"{name}:new_hostgroup{new_hostgroup}")
                    hostgroups.append(new_hostgroup)
                else:
                    self.log.debug(f"{name}:No site?")

                # Tenant
                if nbsrv.tenant and nbsrv.tenant["display"]:
                    self.log.debug(f"{name}:adding hostgroup {nbsrv.tenant['display']}")
                ...
                self.log.debug(f"{name}:setting hostgroups: {hostgroups}")
    ...
    def disable_enable_zabbix_hosts_from_netbox_data(self, zabbix_servers, netbox_servers):
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE:{name}:disable?")
                nbsrv = netbox_servers[name]
                zbsrv = zabbix_servers[name]
                ...
                # Disable if status is various inactive types
                    ...
                    self.log.debug(f"In-Active Host {name}/{nbsrv.status['value']}")
                    self.zabbix.host_disable(zbsrv)
                elif nbsrv.status["value"] in [
                    "planned",
                ]:
                    self.log.debug(f"Ignore {name}/{nbsrv.status['value']}")
                # Per-tenant logic
                elif nbsrv.tenant and nbsrv.tenant["slug"]:
                    # Skip "SOC Special Use" for now
                    if nbsrv.tenant["slug"] == "soc-special-use":
                        self.log.info(f"Skipping SOC Special Use host {name}")
                    # Enable active and staged SOC hosts
                    elif nbsrv.tenant["slug"] == "soc":
                        if nbsrv.status["value"] == "active":
                            self.log.debug(f"SOC Active host {name}")
                            self.zabbix.host_enable(zbsrv)
                        elif nbsrv.status["value"] == "staged":
                            self.log.debug(f"SOC Staged host {name}")
                            self.zabbix.host_enable(zbsrv)
                        else:
                            self.log.info(
                        ...
                        self.log.info(f"Skipping non-SOC host {name}")
                else:
                    self.log.warning(f"No tenant on {name}")

    def copy_netbox_ipmi_to_zabbix(self, netbox_servers, zabbix_servers):
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                if self.config.verbose >= 4:
                    self.log.debug(f"TRACE:{name}:IPMI")
                nbsrv = netbox_servers[name]
                zbsrv = zabbix_servers[name]
                # nbsrv.full_details()
                if self.config.verbose >= 4:
                    self.log.debug(pformat(dict(nbsrv)))
                ipmi_ip = self.netbox.get_ipmi_ip(nbsrv)
                if ipmi_ip:
                    self.log.debug(f"{name}:IPMI IP: {ipmi_ip}")
                    self.zabbix.set_ipmi_interface(zbsrv, ipmi_ip)
                else:
                    self.log.debug(f"{name}: no IPMI IP")
    ...
    def copy_netbox_services_to_zabbix(self, netbox_servers, zabbix_servers):
        ...
        for name in zabbix_servers:
            if name in netbox_servers and netbox_servers[name]:
                if self.config.verbose >= 4:  # 4
                    self.log.debug(f"TRACE:{name}:Services")
                nbsrv = netbox_servers[name]
                # zbsrv = zabbix_servers[name]
                ...
                # self.log.warning(pformat(services))

                open_tcp_ports = []
                open_udp_ports = []
                for service in services:
                    self.log.debug(pformat(dict(service)))
                    if service.protocol.value == "tcp":
                        ...
                        self.log.warning(f"Unknown port protocol {service.protocol.label}")
                        self.log.debug(pformat(service))
    ...
    def create_noc_junipers(self):
        ...
        if self.config.verbose >= 3:
            self.log.debug(f"netbox_juniper_list: {pformat(nbjunipers)}")
        else:
            self.log.debug(f"netbox_juniper_list[0]: {pformat(nbjunipers[0])}")

        # Check tenant, status, tag, etc here.
        for nbjuniper in nbjunipers:
            if self.config.verbose >= 4:
                self.log.debug(f"checking juniper: {pformat(dict(nbjuniper))}")
            else:
                self.log.debug(f"checking juniper {nbjuniper}")
            ...
            if (
                ...
                self.log.info("Skipping because already in zabbix")
            elif any(tag["slug"] == "noc-unmanaged" for tag in nbjuniper.tags):
                self.log.info("Skipping because of noc-unmanaged tag")
            else:
                self.log.info("Creating zabbix host")
                ...
                if nbjuniper.primary_ip and nbjuniper.primary_ip.address:
                    ...
                    self.log.warning(f"No primary IP address on {name}")
                    continue
    ...
    def run(self):
        ...
        if not self.config.skip_creates:
            self.create_hosts_in_zabbix()

        self.log.debug("Getting list(s) of servers from Zabbix")
        zabbix_server_list = self.zabbix.get_hosts_all()
        zabbix_notdiscovered_list = self.zabbix.get_hosts_notdiscovered()
        if self.config.verbose >= 4:
            self.log.debug(f"zabbix_server_list: {pformat(zabbix_server_list)}")
        else:
            self.log.debug(f"zabbix_server_list[0]: {pformat(zabbix_server_list[0])}")
        ...
        self.log.debug("Getting list of servers from Netbox")
        netbox_server_list = self.netbox.get_hosts_all()
        if self.config.verbose >= 4:
            for server in netbox_server_list:
                self.log.debug(f"netbox_server_N: {pformat(dict(server))}")
        else:
